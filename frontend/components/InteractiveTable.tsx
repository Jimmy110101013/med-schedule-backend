...
export default function InteractiveTable({ courses, allExams, onUpdateCourse, focusMode }: InteractiveTableProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!focusMode && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4 bg-zinc-50/80 dark:bg-zinc-900/50 p-4 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm">
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-2">
              <Filter className="w-5 h-5 text-zinc-500" />
              <span className="text-base font-bold text-zinc-700 dark:text-zinc-300">科目：</span>
              <select
                value={selectedCategory}
                onChange={(e) => setSelectedCategory(e.target.value)}
                className="bg-white dark:bg-zinc-700 border border-zinc-300 dark:border-zinc-600 text-base rounded-lg px-4 py-2 focus:ring-2 focus:ring-zinc-900 outline-none cursor-pointer text-zinc-800 dark:text-zinc-200 font-medium min-w-[200px]"
              >
                {uniqueCategories.map(cat => <option key={cat} value={cat}>{cat === "All" ? "顯示所有科目" : cat}</option>)}
              </select>
            </div>

            <div className="flex items-center gap-2 sm:border-l-2 sm:border-zinc-200 dark:sm:border-zinc-700 sm:pl-6">
              <Target className="w-5 h-5 text-orange-500" />
              <span className="text-base font-bold text-zinc-700 dark:text-zinc-300">考試目標：</span>
              <select
                value={selectedExam}
                onChange={(e) => setSelectedExam(e.target.value)}
                className="bg-white dark:bg-zinc-700 border border-zinc-300 dark:border-zinc-600 text-base rounded-lg px-4 py-2 focus:ring-2 focus:ring-orange-500 outline-none cursor-pointer text-zinc-800 dark:text-zinc-200 font-medium min-w-[280px] truncate"
              >
          ...
          <div className="flex items-center gap-3 bg-white dark:bg-zinc-800 px-4 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 shadow-sm">
            <Activity className="w-5 h-5 text-green-500" />
            <span className="text-base font-bold text-zinc-600 dark:text-zinc-300">共 <span className="text-zinc-900 dark:text-zinc-100 text-lg">{totalFiltered}</span> 堂</span>
            <span className="text-zinc-300 dark:text-zinc-600 text-lg">|</span>
            <span className="text-base font-bold text-zinc-600 dark:text-zinc-300">已掌握 <span className="text-green-600 dark:text-green-400 text-lg">{studiedFiltered}</span> 堂</span>
        ...
        <Table>
          <TableHeader className="bg-zinc-100/50 dark:bg-zinc-700/50">
            <TableRow className="hover:bg-transparent dark:hover:bg-transparent">
              <TableHead className="w-[120px] text-base font-bold text-zinc-700 dark:text-zinc-300 py-4">日期</TableHead>
              <TableHead className="w-[120px] text-base font-bold text-zinc-700 dark:text-zinc-300 py-4">時段</TableHead>
              <TableHead className="w-[160px] text-base font-bold text-zinc-700 dark:text-zinc-300 py-4">科目</TableHead>
              <TableHead className="text-base font-bold text-zinc-700 dark:text-zinc-300 py-4">課程主題</TableHead>
              <TableHead className="w-[240px] text-base font-bold text-zinc-700 dark:text-zinc-300 py-4">對應考試 (可點擊微調)</TableHead>
              <TableHead className="w-[150px] text-base font-bold text-zinc-700 dark:text-zinc-300 py-4">出席狀態</TableHead>
              <TableHead className="w-[220px] text-base font-bold text-zinc-700 dark:text-zinc-300 py-4">內化進度</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
                ...
                <TableCell className="py-4">
                  <div className="flex items-center gap-3">
                    {['一刷', '二刷', '寫考古'].map(type => {
                      ...
                      return (
                        <label key={type} className={`flex items-center gap-1.5 text-[15px] cursor-pointer transition-colors ${checked ? 'text-green-700 dark:text-green-400 font-extrabold' : 'text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200 font-bold'}`}>
                          <input
                            type="checkbox"
                            checked={checked}
