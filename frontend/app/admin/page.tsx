...
export default function AdminDashboard() {
  ...
  return (
    <main className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-8 text-zinc-900 dark:text-zinc-100 font-sans">
      <Toaster position="bottom-right" />
      <div className="max-w-4xl mx-auto space-y-8">
        <header className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-700 pb-6 pt-4">
          <div><h1 className="text-3xl font-bold flex items-center gap-2"><Settings className="w-8 h-8 text-zinc-700 dark:text-zinc-300" /> 系統管理中心</h1><p className="text-zinc-500 dark:text-zinc-400">動態設定考試範圍與課表資料庫</p></div>
          <Link href="/" className="text-sm font-medium text-blue-600 flex items-center gap-2"><ArrowLeft className="w-4 h-4" /> 返回戰略儀表板</Link>
        </header>

        <Card className="shadow-sm border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/80">
          <CardHeader><CardTitle className="text-lg flex items-center gap-2 dark:text-zinc-100"><UploadCloud className="w-5 h-5 text-blue-500" /> 匯入新學期 PDF 課表</CardTitle></CardHeader>
          <CardContent><div className="border-2 border-dashed border-zinc-300 dark:border-zinc-600 rounded-lg p-8 text-center bg-zinc-50 dark:bg-zinc-700/30"><p className="text-zinc-500 dark:text-zinc-400 text-sm font-medium">功能建置中：即將整合 PDF 解析引擎</p></div></CardContent>
        </Card>

        <Card className="shadow-sm border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800/80">
          <CardHeader><CardTitle className="text-lg dark:text-zinc-100">{editingId ? "📝 編輯考試映射規則" : "➕ 新增區段考範圍規則"}</CardTitle></CardHeader>
          <CardContent className="flex gap-4">
            <input type="text" placeholder="關鍵字 (例: 骨)" value={newKeyword} onChange={e => setNewKeyword(e.target.value)} className="border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 rounded-md px-3 py-2 w-1/3 text-sm font-medium" />
        ...
        <div className="bg-white dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-700 rounded-xl p-6">
          <h3 className="text-lg font-bold mb-4 dark:text-zinc-100">目前生效中的映射字典</h3>
          <div className="space-y-4">
            {rules.map((rule) => (
              <div key={rule.id} className="flex justify-between items-start border-b border-zinc-100 dark:border-zinc-700 pb-4">
                <div>
                  <div className="font-bold text-lg text-zinc-800 dark:text-zinc-200 flex items-center gap-2">關鍵字：<span className="text-orange-600 dark:text-orange-400">{rule.keyword}</span></div>
                  <div className="mt-2 flex flex-wrap gap-2">{rule.categories.map(cat => <Badge key={cat} variant="secondary" className="bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 font-normal">{cat}</Badge>)}</div>
                </div>
                <div className="flex gap-2">
